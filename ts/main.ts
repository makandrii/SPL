...
export const findAvailableClassroom = (
	timeSlot: TimeSlot,
	dayOfWeek: DayOfWeek,
): string[] => {
	...
	return classrooms
		...
		.filter(
			(classroomNumber: string) =>
				!occupiedClassrooms.includes(classroomNumber),
		);
};

export const getProfessorSchedule = (professorId: number): Lesson[] => {
	return schedule.filter(
		(lesson: Lesson) => lesson.professorId === professorId,
	);
};

export const validateLesson = (lesson: Lesson): ScheduleConflict | null => {
	const firstConflict: Lesson | undefined = schedule.find(
	...
	if (firstConflict) {
		const type: 'ProfessorConflict' | 'ClassroomConflict' =
			firstConflict.professorId === lesson.professorId
				? 'ProfessorConflict'
				: 'ClassroomConflict';
...
export const getClassroomUtilization = (classroomNumber: string): number => {
	const totalLessons: number = schedule.filter(
		(lesson: Lesson) => lesson.classroomNumber === classroomNumber,
	).length;
...
export const getMostPopularCourseType = (): CourseType => {
	...
	return Object.keys(courseTypeCount).reduce((a, b) =>
		courseTypeCount[a as CourseType]! > courseTypeCount[b as CourseType]!
			? a
			: b,
	) as CourseType;
};

export const reassingClassroom = (
	lessonId: number,
	newClassroomNumber: string,
): boolean => {
	const lesson: Lesson | undefined = schedule.find(
		(lesson: Lesson) => lesson.lessonId === lessonId,
	);
	if (
		lesson &&
		validateLesson({
